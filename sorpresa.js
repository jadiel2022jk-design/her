...
let currentGallery = null; // Para saber de cuál persona vinimos
...
btnEnter.addEventListener('click', () => {
  goTo('screen-welcome', 'screen-stats');
});
...
btnNext.addEventListener('click', () => {
  idx++;
  if (idx >= MAX) { 
    goTo('screen-stats', 'screen-anime');
    return; 
  }
...
// ── Anime avatares (invertidos: anime_a = Tu, anime_b = Yo) ─
document.getElementById('animeA').src = FOTOS.anime_b;
document.getElementById('animeB').src = FOTOS.anime_a;

// ── Función para ir a galería específica ──
function goToGallery(person) {
  currentGallery = person;
  if (person === 'a') {
    goTo('screen-anime', 'screen-fotos-a');
  } else {
    goTo('screen-anime', 'screen-fotos-b');
  }
}

// ── Función para ir a mensaje ──
function goToMessage(fromScreen) {
  goTo(fromScreen, 'screen-msg');
}
// ── Anime avatares (invertidos: anime_a = Tu, anime_b = Yo) ─
document.getElementById('animeA').src = FOTOS.anime_b;
document.getElementById('animeB').src = FOTOS.anime_a;

// ── Función para ir a galería o mensaje ──
function goToGallery(person) {

  // Perfil izquierdo → mensaje
  if (person === 'a') {
    goTo('screen-anime', 'screen-msg');
    return;
  }

  // Perfil derecho → fotos
  goTo('screen-anime', 'screen-fotos-b');
}
...
// ── Galería B (única galería utilizada) ──
const galeria_b = [
  FOTOS.jess,
  FOTOS.img1,
  FOTOS.img2,
...
// Crear galería B
const fotoGridB = document.getElementById('fotoGridB');

galeria_b.forEach((src, i) => {
  ...
  fotoGridB.appendChild(wrap);
});

// ── Navegación mejorada (arregla pointer-events) ────
...
// ── Función de volver atrás mejorada ──
function goBack(fromId, toId) {
  ...
  from.classList.remove('active');
  from.style.opacity       = '0';
  from.style.transform     = 'translateY(18px)';
